import { Restaurant } from '../types/restaurants/get-by-location';
import {
  GetRestaurantProductsResponse,
  ScrapedCategory,
  ScrapedProduct,
} from '../types/restaurants/get-restaurant-products';
import { RestaurantSearchResult } from '../types/restaurants/search-by-location';

export function filteredRestaurantToLLMString(
  r: Restaurant,
  index: number
): string {
  const status = r.isClosed ? 'Currently closed' : 'Open now';
  const promoText =
    r.promotions?.map((p) => `- ${p.promotionName}`).join('\n') ||
    'No promotions';
...
export function searchedRestaurantToLLMString(
  r: RestaurantSearchResult,
  index: number
): string {
  ...
  const deliveryFeesText = r.deliveryFees?.length
    ? r.deliveryFees
        .map(
          (fee) =>
            `Min: ₺${fee.minBasketPrice}, Max: ₺${fee.maxBasketPrice}, Shipping: ₺${fee.shippingPrice}`
        )
        .join(' | ')
...
export function restaurantProductsToLLMString(
  products: GetRestaurantProductsResponse
): string {
  return `
...
export function scrapedProductsToLLMString(category: ScrapedCategory): string {
  const productsText = category.products
    .map(
      (p: ScrapedProduct, i: number) => `
Product ${i + 1}: ${p.title}
